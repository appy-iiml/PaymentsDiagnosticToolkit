...
def create_radar_chart(banks_data,
                       selected_banks,
                       title="Bank Capability Comparison"):
    """
...
def create_stage_chart(bank_data, stage_id, comparison_banks=None):
    ...
    if bank_data:
        # Calculate total score for this stage
        stage_total = 0
        process_count = 0

        # Get processes for this stage
        from data_loader import get_process_data_for_stage
        stage_processes = get_process_data_for_stage(stage_id)

        # Sum the scores for each process in this stage
        for process in stage_processes:
            if process['qid'] in bank_data:
                stage_total += bank_data[process['qid']]
                process_count += 1
        ...
        fig.add_trace(
            go.Bar(
                ...
                marker_color='#A100FF'  # Updated to match Accenture purple
            ))
    ...
    if comparison_banks:
        for bank, score in comparison_banks.items():
            if score.get(stage_id):
                fig.add_trace(go.Bar(x=[bank], y=[score[stage_id]], name=bank))
...
def create_outcome_chart(outcome_data):
    ...
    fig.add_trace(
        go.Scatterpolar(
            ...
            fillcolor="rgba(161, 0, 255, 0.3)",  # Light purple fill
            line=dict(color="#A100FF")  # Accenture purple line
        ))
...
def create_maturity_heatmap(banks_data, selected_banks):
    ...
    fig = go.Figure(data=go.Heatmap(
        ...
        y=selected_banks,
        colorscale=[
            [0, "#F8F5FF"],  # Very light purple for low values
            [0.33, "#D6C4FF"],  # Light purple
            [0.66, "#B967FF"],  # Medium purple
            [1, "#A100FF"]  # Accenture purple for high values
        ],
        hoverongaps=False,
        colorbar=dict(title="Maturity<br>Score",
                      tickmode="array",
                      tickvals=[0, 1, 2, 3, 4],
                      ticktext=["0", "1", "2", "3", "4"],
                      ticks="outside",
    ...
    fig.update_layout(title="Payment Capability Maturity Heatmap",
                      height=400,
