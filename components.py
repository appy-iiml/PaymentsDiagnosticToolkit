...
def render_comparison_view(banks):
    ...
    st.markdown("### Competitor Selection")
    show_all = st.checkbox("Show all banks in comparison", value=False)
    ...
    if not selected_banks:
        st.warning("Please select at least one bank for comparison")
        selected_banks = all_options[:1]
    
    # Create visualization tabs
    tab1, tab2, tab3 = st.tabs(["Radar Chart", "Heatmap", "Stage Breakdown"])
    ...
    with tab2:
        ...
        st.markdown("""
        **Legend:**
        - 0-1: Basic capabilities
        - 1-2: Advanced capabilities
        - 2-3: Leading capabilities
    ...
    with tab3:
        ...
        # Create comparison data for selected stage
        stage_data = {}
        for bank in selected_banks:
            if bank in combined_data:
                stage_data[bank] = combined_data[bank].get(selected_stage, 0)
        
        # Create DataFrame for display
        stage_df = pd.DataFrame({
            "Bank": list(stage_data.keys()),
            f"Stage {selected_stage} Score": list(stage_data.values())
        })
        
        # Sort by score (descending)
        stage_df = stage_df.sort_values(f"Stage {selected_stage} Score", ascending=False)
        
        # Display as table
        st.dataframe(stage_df, use_container_width=True)
        
        # Create bar chart for the stage
        import plotly.express as px
        fig = px.bar(
            stage_df, 
            x="Bank", 
            y=f"Stage {selected_stage} Score",
            title=f"Stage {selected_stage}: {stage_names[selected_stage]} - Bank Comparison",
            color=f"Stage {selected_stage} Score",
            color_continuous_scale=["#F8F5FF", "#D6C4FF", "#B967FF", "#A100FF"]
        )
        fig.update_layout(yaxis_range=[0, 4])
        st.plotly_chart(fig, use_container_width=True)

# This function has been replaced by the new integrated version below
